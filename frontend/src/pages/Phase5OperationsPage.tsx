...
function ScoreGauge({ score }: { score: number }) {
  ...
  return (
    <svg width="164" height="92" viewBox="0 0 164 92" aria-label={`Risk posture score ${clamped}`}>
      <path d="M 12 84 A 70 70 0 0 1 152 84" fill="none" stroke="#1e293b" strokeWidth="14" strokeLinecap="round" />
      <path
...
function TrendChart({ values }: { values: number[] }) {
  ...
  return (
    <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`} aria-label="Risk trend chart">
...
export function Phase5OperationsPage() {
  ...
  const liveAlerts = (alerts.data ?? MOCK_ALERTS).filter((alert) => !dismissedAlertIds.includes(alert.id));
  ...
  const openIncidents = timelineIncidents.filter((incident) => incident.status !== "resolved").length;
  const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const closedThisWeek = timelineIncidents.filter(
    (incident) => incident.status === "resolved" && new Date(incident.timestamp).getTime() >= weekAgo,
  ).length;
      ...
      <div className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-400">
            Unified command center for risk posture, SOC operations, incidents, CI gates, and bug bounty performance.
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="grid gap-4 sm:grid-cols-[170px_1fr] sm:items-center">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400">
                  <span>Benchmark percentile: {riskPosture.data?.industry_benchmark_percentile ?? 68}%</span>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900 p-4">
              ...
              <table className="w-full text-left text-sm">
                <tbody className="divide-y divide-slate-800">
                  <tr>
                    ...
                    <td className="px-3 py-2 text-slate-200">{socMetrics.data?.mttd_minutes ?? 14} min</td>
                  </tr>
                  <tr>
                    <td className="px-3 py-2 text-slate-400">MTTR</td>
                    <td className="px-3 py-2 text-slate-200">{socMetrics.data?.mttr_minutes ?? 42} min</td>
                  </tr>
                  <tr>
            ...
            <p className="mt-3 text-xs text-slate-500">
              Total alerts: {socDashboard.data?.total_alerts ?? 32} · Critical: {socDashboard.data?.critical_alerts ?? 6} ·
              Avg priority: {socDashboard.data?.average_priority ?? 74}
            </p>
          </section>
        </div>
          ...
          <section className="rounded-xl border border-slate-800 bg-slate-900 p-4">
            ...
            <div className="space-y-2">
              {liveAlerts.map((alert) => (
                <article key={alert.id} className="rounded-lg border border-slate-800 bg-slate-950 p-3">
                  <div className="mb-1 flex items-start justify-between gap-2">
                    <p className={`text-sm font-medium ${severityClass(alert.severity)}`}>{alert.title}</p>
                    <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase ${statusClass(alert.status)}`}>
                  ...
                  <div className="mt-2 flex gap-2">
                    <button
                      type="button"
                      onClick={() => setAcknowledgedAlertIds((previous) => [...new Set([...previous, alert.id])])}
                    ...
                    <button
                      type="button"
                      onClick={() => setDismissedAlertIds((previous) => [...new Set([...previous, alert.id])])}
            ...
            {!alerts.data && alerts.error ? (
              <p className="mt-3 text-xs text-amber-300">Live alert feed unavailable; showing cached SOC snapshot.</p>
            ) : null}
          </section>
            ...
            <div className="space-y-3">
              {timelineIncidents.map((incident) => (
                <div key={incident.id} className="relative pl-6">
                  ...
                  <span className={`absolute left-0 top-1.5 h-3 w-3 rounded-full ${severityClass(incident.severity).replace("text", "bg")}`} />
                  <div className="rounded-lg border border-slate-800 bg-slate-950 p-3">
                    <div className="mb-1 flex items-center justify-between gap-2">
                      <p className="text-sm font-medium text-slate-200">{incident.title}</p>
                      <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase ${statusClass(incident.status)}`}>
                        {incident.status}
                      </span>
                    </div>
                ...
                <tbody className="divide-y divide-slate-800">
                  {ciRows.map((row) => (
                    <tr key={row.id}>
                      ...
                      <td className="px-3 py-2 text-slate-400">{Math.round(row.runtime_seconds / 60)} min</td>
                    </tr>
                  ))}
                </tbody>
              </table>
                  ...
                  <div key={entry.month} className="flex flex-col items-center gap-1">
                    <div className="h-24 w-full rounded bg-slate-800">
                      <div
                        ...
                        style={{ height: `${Math.max(8, Math.round((entry.amount / maxMonthlyEarning) * 100))}%` }}
                      />
                    </div>
