...
_PRIVILEGED_CMDS = re.compile(
    r"\b(chmod\s+777|chmod\s+-R\s+777|sudo\s+|--privileged)\b", re.IGNORECASE
)
    ...
    for doc in docs:
        ...
        if kind in {"Deployment", "DaemonSet", "StatefulSet", "Job", "ReplicaSet", "Pod"}:
            ...
            for subject in doc.get("subjects", []):
                if (
                    subject.get("name") not in {"system:masters"}
                    and doc.get("roleRef", {}).get("name") == "cluster-admin"
                ):
                    findings.append(
                        _finding(
            ...
            if not annotations.get(
                "nginx.ingress.kubernetes.io/ssl-redirect"
            ) and not annotations.get("kubernetes.io/tls"):
                findings.append(
                    _finding(
...
def analyze_dockerfile(dockerfile_content: str) -> dict[str, Any]:
    ...
    for idx, raw_line in enumerate(lines, start=1):
        ...
        if instruction == "USER":
            ...
            if ":" not in img_clean and "@" not in img_clean:
                ...
                findings.append(
                    _finding(
                        ...
                        description="Using ':latest' tag means base image can change silently between builds.",
                        severity="medium",
            ...
            if (
                re.search(r"\bapt-get\s+install\b", rest)
