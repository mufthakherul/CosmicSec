...
"""
    ...
    if args.enable_ssl:
        env_content += f"""TLS_CERT_PATH=/etc/cosmicsec/certs/tls.crt
...
def create_systemd_service(args: argparse.Namespace) -> Optional[Path]:
...
"""
    ...
    print(f"  sudo mkdir -p /etc/systemd/system")
    print(f"  sudo tee {service_file} > /dev/null << 'EOF'")
    print(service_content)
    print("EOF")
    print(f"  sudo systemctl daemon-reload")
    print(f"  sudo systemctl enable cosmicsec")
    print(f"  sudo systemctl start cosmicsec")
...
def create_firewall_rules(args: argparse.Namespace):
    ...
    if os_name == "Linux":
        ...
        print(f"  sudo ufw allow 80/tcp   # HTTP redirect")
        if args.enable_ssl:
            print(f"  sudo ufw allow 443/tcp  # HTTPS")
        print()
        print("For iptables:")
        ...
        print(f"  New-NetFirewallRule -DisplayName 'CosmicSec' -Direction Inbound -Action Allow -Protocol TCP -LocalPort {args.port}")
...
def create_reverse_proxy_config(args: argparse.Namespace) -> Optional[Path]:
    """Create Nginx reverse proxy configuration."""
    print_header("Creating Reverse Proxy Configuration")
    
    nginx_config = f"""# Nginx configuration for CosmicSec
...
"""
    ...
    print(f"  sudo ln -s /etc/nginx/sites-available/cosmicsec /etc/nginx/sites-enabled/")
    print(f"  sudo systemctl restart nginx")
...
Examples:
  ...
  python scripts/setup-self-hosted.py --domain cosmicsec.example.com --enable-ssl
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help="External port (default: 8000)"
    )
    parser.add_argument(
        "--enable-ssl",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-docker-check",
        action="store_true",
        help="Skip Docker prerequisite check"
    )
    ...
    print(f"   docker compose logs -f api-gateway")
    print()
    ...
    print()
    print(f"{Colors.YELLOW}⚠️  IMPORTANT: Change the admin password immediately after first login!{Colors.ENDC}")
    print()
